...
* Graphical problems
In case problems, try disabling the compositor in desktop settings and re-login. I specifically had problems with xfwm4 (the XFCE window manager) and disabling the compositor didn't help, so I switched back to KDE Plasma, which is lightweight these days (didn't even need to disable compositor with KDE).
