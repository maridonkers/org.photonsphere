...
Use =make database-ls= to see if a database is running an to see its id. To stop it use a =docker stop id= command (use a =docker start id= command to start it again). To remove it use a =docker rm id= command.
