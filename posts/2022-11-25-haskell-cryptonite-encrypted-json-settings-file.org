...
This code appears to work, but I have not battle tested it, hence no guarantees are given at all! Specifically the =paramN=, =paramR= and =paramP= values for =deriveKey= may need revising — see the [[https://hackage.haskell.org/package/cryptonite-0.30/docs/Crypto-KDF-Scrypt.html][Crypto.KDF.Scrypt]] documentation — and also it's probably not wise to keep decrypted fields in memory (i.e. decrypt every time into ephemeral values when encrypted fields are required).
