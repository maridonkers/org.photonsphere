...
This code appears to work, but I have not battle tested it, hence no guarantees are given at all! Specifically the =params= for =deriveKey= may need revising and also it's not wise to keep decrypted fields in memory (i.e. decrypt every time into ephemeral values when encrypted fields are required).
